...
szse_monitor.py
監測深圳證券交易所「发行H股」相關公告。
直接呼叫深交所 POST JSON API，無需解析 HTML。
比對「證券代碼」是否已存在於 szse_records.json，若未見過則視為新增。
szse_records.json 最多保留 1000 筆記錄。
"""
...
RECORDS_FILE    = "szse_records.json"
...
# 深交所 POST JSON API（你親測有效）
SZSE_API_URL = "http://www.szse.cn/api/disc/announcement/annList?random=0.123456"
...
def fetch_szse_announcements() -> list[dict]:
    ...
    for attempt in range(3):
        try:
            logger.info("深交所 API 請求（嘗試 %d）…", attempt + 1)
            resp = requests.post(
                SZSE_API_URL,
                json=payload,
                headers=headers,
            ...
            for row in rows:
                ...
                sec_code = (row.get("secCode") or row.get("stockCode") or "").strip()
                sec_name = (row.get("secName") or row.get("stockName") or "").strip()
                title    = (row.get("title")   or row.get("announcementTitle") or "").strip()
...
def main() -> None:
    ...
    announcements = fetch_szse_announcements()

    if not announcements:
        logger.info("深交所未抓取到任何符合條件公告，本次執行結束。")
        return
    ...
    push_to_github(
        RECORDS_FILE,
        f"chore: 更新深交所記錄 {today_hk_str} [{len(newly_added)} 筆新增]",
    )
